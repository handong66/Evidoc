...
function formatPrRepairGuidance(report: DriftReport, options: ReportFormatOptions): string[] {
  ...
  const lines = [
    ...
    "2. Apply deterministic safe fixes locally: `npx repo-evidoc fix --safe --write --json --root <target-repository-root>`",
    "   Source checkout: `npm run evidoc -- fix --safe --write --json --root <target-repository-root>`",
    "3. Generate evidence-bound repair prompts: `npx repo-evidoc diagnose --root <target-repository-root>`",
    "   Source checkout: `npm run evidoc -- diagnose --root <target-repository-root>`",
    "4. Re-run Evidoc before merging: `npx repo-evidoc check --fail-on=review_needed --root <target-repository-root>`",
