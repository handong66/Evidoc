...
function formatPrRepairGuidance(report: DriftReport, options: ReportFormatOptions): string[] {
  ...
  const lines = [
    ...
    "",
    "Replace `<target-repository-root>` with the repository root being repaired. Local `npx @evidoc/evidoc` commands use the published npm package. When testing unreleased changes from an Evidoc source checkout, use the source-checkout fallback shown under each command.",
    "",
    "1. Preview deterministic safe fixes: `npx @evidoc/evidoc fix --safe --json --root <target-repository-root>`",
    "   Source checkout: `npm run evidoc -- fix --safe --json --root <target-repository-root>`",
    "2. Apply deterministic safe fixes locally: `npx @evidoc/evidoc fix --safe --write --json --root <target-repository-root>`",
    "   Source checkout: `npm run evidoc -- fix --safe --write --json --root <target-repository-root>`",
    "3. Generate evidence-bound repair prompts: `npx @evidoc/evidoc diagnose --root <target-repository-root>`",
    "   Source checkout: `npm run evidoc -- diagnose --root <target-repository-root>`",
    "4. Re-run Evidoc before merging: `npx @evidoc/evidoc check --fail-on=review_needed --root <target-repository-root>`",
...
function formatNoDocumentCoverageGuidance(): string[] {
  return [
    ...
    "- Initialize repository support files with `npx @evidoc/evidoc init --yes --root <target-repository-root>`.",
    "- When testing from an Evidoc source checkout, run `npm run evidoc -- init --yes --root <target-repository-root>`.",
    "- Re-run Evidoc before merging: `npx @evidoc/evidoc check --fail-on=review_needed --root <target-repository-root>` or `npm run evidoc -- check --fail-on=review_needed --root <target-repository-root>` from a source checkout.",
    ""
  ];
}
...
function formatAgentRepairPrompt(findings: DriftFinding[], safeFindings: DriftFinding[], repositoryRoot: string): string[] {
  ...
  return [
    ...
    "",
    "Repair commands:",
    "- Preview deterministic safe fixes: `npx @evidoc/evidoc fix --safe --json --root <target-repository-root>`",
    "- Apply deterministic safe fixes: `npx @evidoc/evidoc fix --safe --write --json --root <target-repository-root>`",
    "- Generate evidence-bound repair prompts: `npx @evidoc/evidoc diagnose --root <target-repository-root>`",
    "- Source-checkout fallback: replace `npx @evidoc/evidoc` with `npm run evidoc --` when testing from an Evidoc source checkout.",
    "",
    ...
    "",
    "Verification:",
    "- Run `npx @evidoc/evidoc check --fail-on=review_needed --root <target-repository-root>`.",
