    ...
    return Promise.all([
      writeScaffoldFile(
        ...
        [
          ...
          "",
          "- Run `npx @evidoc/evidoc guard --event manual --scope staged --json` before final code, doc, review, or commit summaries.",
      ...
      writeScaffoldFile(
        ...
        [
          ...
          "Recommended loop:",
          "",
          "1. `npx @evidoc/evidoc guard --event manual --scope staged --json`",
          "2. Check `runtime.status`, `runtime.fingerprint`, and `runtime.findings[].fingerprint`.",
          "3. `npx @evidoc/evidoc diagnose` before proposing repairs.",
...
function localGitHookScript(event: "pre-commit" | "pre-push", baseBranch: string): string {
  ...
  return [
    ...
    `  echo "Evidoc ${event} skipped: install Node.js plus Evidoc, or run npx @evidoc/evidoc ${fallbackCommand}." >&2`,
    "  exit 0",
    "elif [ -x ./node_modules/.bin/evidoc ]; then",
    `  ./node_modules/.bin/evidoc ${guardCommand}`,
    "elif command -v npx >/dev/null 2>&1; then",
    `  npx --yes evidoc ${guardCommand}`,
...
function agentInstructions(surface: string): string {
  return [
    ...
    "- Before finalizing code changes, documentation changes, review summaries, or commit summaries, run `npx @evidoc/evidoc guard --event manual --scope staged --json` from the repository root.",
    ...
    "- When findings exist, report affected documents, evidence-backed cause, and next commands: `npx @evidoc/evidoc diagnose`, `npx @evidoc/evidoc fix --safe --write`, and `git diff`.",
...
function evidocSkillInstructions(): string {
  return [
    ...
    "3. Otherwise run `npx @evidoc/evidoc guard --event manual --scope staged --json`.",
    "4. If `evidoc.diagnose_drift` is listed, call it before proposing repairs; otherwise use `npx @evidoc/evidoc diagnose`.",
    "",
    "## Response Contract",
    "",
    ...
    "- Use `evidoc.suggest_doc_fix` or `npx @evidoc/evidoc fix --safe --write` only for evidence-bound safe fixes.",
