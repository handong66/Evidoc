...
function formatInitResult(result: InitResult, root: string): string {
  ...
  lines.push(
    ...
    result.localGit
      ...
      : `  npx @evidoc/evidoc check --root ${quoteShellArg(root)} --fail-on=review_needed`,
    result.localGit
      ? "  npx @evidoc/evidoc guard --event pre-commit --root " + quoteShellArg(root)
      : "",
...
function formatDoctorResult(result: OnboardingInspection, root: string): string {
  ...
  if (!result.ready) {
    lines.push(
      "",
      "Fix:",
      `  npx @evidoc/evidoc init --yes --root ${quoteShellArg(root)}`,
      `  npx @evidoc/evidoc init --yes --local-git --install-hooks --root ${quoteShellArg(root)}`,
      "Source checkout fallback:",
  ...
  const all: CiRecipe[] = [
    {
      ...
      content: [
        ...
        "    - npx @evidoc/evidoc guard --event pre-push --since merge-base:$EVIDOC_BASE_BRANCH --fail-on=review_needed",
        ""
      ].join("\n")
    },
    {
      ...
      content: [
        "stage('Evidoc') {",
        "  sh 'npm ci'",
        "  sh 'EVIDOC_BASE_BRANCH=${CHANGE_TARGET:-main} npx @evidoc/evidoc guard --event pre-push --since merge-base:$EVIDOC_BASE_BRANCH --fail-on=review_needed'",
        "}",
        ""
      ].join("\n")
    },
    {
      ...
      content: [
        ...
        "          node-version: 22",
        "      - run: npm ci",
        "      - run: npx @evidoc/evidoc guard --event pre-push --since merge-base:${EVIDOC_BASE_BRANCH:-main} --fail-on=review_needed",
        ""
      ].join("\n")
    },
    {
