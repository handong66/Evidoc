...
test("init creates config and GitHub workflow for zero-friction onboarding", async () => {
  ...
  assert.match(stdout, /npx @evidoc\/evidoc check/);
  assert.match(stdout, /Source checkout fallback/);
  assert.ok(stdout.includes(`npx @evidoc/evidoc check --root ${shellArg(root)} --fail-on=review_needed`));
...
test("doctor reports missing and ready onboarding state", async () => {
  ...
  assert.match(stdout, /npx @evidoc\/evidoc init --yes/);
  assert.match(stdout, /Source checkout fallback/);
  assert.ok(stdout.includes(`npx @evidoc/evidoc init --yes --root ${shellArg(root)}`));
