...
`npx @evidoc/evidoc` and `npx @evidoc/evidoc init --yes` also maintain `.evidoc/.gitignore` with `history.jsonl` and `reports/` so local Command Center scan history and Local Git Gate reports stay out of commits. Commit the config and `.evidoc/.gitignore`; leave generated history and reports local.
