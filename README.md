...
[![npm](https://img.shields.io/npm/v/%40evidoc%2Fevidoc?style=flat-square)](https://www.npmjs.com/package/@evidoc/evidoc)
[![Node.js Version](https://img.shields.io/node/v/%40evidoc%2Fevidoc?style=flat-square)](https://nodejs.org)
...
[![License](https://img.shields.io/npm/l/%40evidoc%2Fevidoc?style=flat-square)](./LICENSE)
...
This first check is read-only. If it finds drift, inspect evidence with `npx @evidoc/evidoc diagnose`, apply only deterministic fixes with `npx @evidoc/evidoc fix --safe --write --json`, or open the loopback-only Local App with `npx @evidoc/evidoc app`.
...
The umbrella npm package is `@evidoc/evidoc`; its installed command is `evidoc`.
For one-off use, type `npx @evidoc/evidoc ...`.

Choose one adoption path after the read-only check:

| I want to... | Start with |
|--------------|------------|
| Give a coding agent current repo evidence | Configure the MCP server or run `npx @evidoc/evidoc init --yes --with agents` |
| See findings, history, gates, and fixes in a browser | `npx @evidoc/evidoc app` |
| Add a local or PR gate | `npx @evidoc/evidoc init --yes --local-git --install-hooks` or `npx @evidoc/evidoc init --yes` |

Try a safe sample first with `npx @evidoc/evidoc demo`.

Common commands:

| Goal | Command |
|------|---------|
| Check setup | `npx @evidoc/evidoc doctor` |
| Scan docs and agent surfaces | `npx @evidoc/evidoc check --fail-on=review_needed` |
| Open the bilingual Command Center | `npx @evidoc/evidoc app` |
| Generate repair prompts for Codex, Claude Code, OpenCode, or another agent | `npx @evidoc/evidoc diagnose` |
| Check AGENTS.md, CLAUDE.md, Cursor rules, and Copilot instructions | `npx @evidoc/evidoc verify --instructions --json` |
| Use MCP from an agent | `evidoc.agent_scan`, then `evidoc.get_drift_status`, `evidoc.diagnose_drift`, or `evidoc.suggest_doc_fix` |
| Export graph/report data | `npx @evidoc/evidoc graph --json`, `npx @evidoc/evidoc index --json`, or `npx @evidoc/evidoc dashboard --out evidoc.html` |
| Apply deterministic safe fixes only | `npx @evidoc/evidoc fix --safe --write --json` |
| Check a branch before push | `npx @evidoc/evidoc guard --event pre-push --since merge-base:main` |
| Generate GitLab, Jenkins, Gitea, or Buildkite snippets | `npx @evidoc/evidoc recipes --target all` |
...
|---------|---------------|-------------|
| `npx @evidoc/evidoc check --fail-on=review_needed` | No | You want a terminal report. |
| `npx @evidoc/evidoc doctor` | No | You want setup diagnostics. |
| `npx @evidoc/evidoc diagnose` | No | You want evidence-bound repair prompts. |
| `npx @evidoc/evidoc app` | May create `.evidoc/config.json` and `.evidoc/.gitignore` if missing. | You want the local Command Center. |
| `npx @evidoc/evidoc init --yes` | Yes | You want config plus a GitHub Actions workflow. |
| `npx @evidoc/evidoc init --yes --local-git --install-hooks` | Yes | You want repo-local Git hooks instead of GitHub Actions. |
| `npx @evidoc/evidoc fix --safe --write --json` | Yes | You want deterministic safe fixes applied. |
...
In non-interactive shells, bare `npx @evidoc/evidoc` runs a one-shot local app scan instead of opening a browser or keeping a server alive.
...
| `review_needed` | A doc or agent instruction may be stale and needs human or agent review. | Run `npx @evidoc/evidoc diagnose` or open `npx @evidoc/evidoc app`. |
...
| Symptom | What to try |
|---------|-------------|
| `npx @evidoc/evidoc` fails before running | Check `node --version`; npm installs require Node.js 22 or later. |
| `doctor` is not ready | Run `npx @evidoc/evidoc init --yes` or `npx @evidoc/evidoc init --yes --local-git --install-hooks`. |
...
| An agent report feels stale | Re-run `npx @evidoc/evidoc guard --event manual --scope staged --json` and compare runtime fingerprints. |
